...
export class AddProductComponent implements OnInit, AfterViewInit {
    ...
    setForm(product) {
        this.form.setValue({
            ...product,
            hidden: product.hidden ? product.hidden : null,
    ...
    onImageInputChange(e) {
        ...
        this.addImageStorage(e.target.files[0])
            .then((downloadUrl: string) => {
                ...
                this.product.imageUrls.push(downloadUrl);
                return downloadUrl
            })
    ...
    removeLocalUrl(doomedUrl: string) {
        console.log(this.product.imageUrls);
        console.log(doomedUrl)
        const index = this.product.imageUrls.findIndex((url: string) => {
            return url === doomedUrl
        })
        console.log(index)
    ...
    updateLocalImageUrls(imageUrls) {
        ...
        if (!this.editmode) {
            ...
            this.fsService.addDoc(pathToProducts, product)
                .then((docRef: DocumentReference) => {
                    ...
                    this.form.patchValue({ id: this.product.id })
    ...
    onCancel() {
        if (this.formChanged) {
            const confirmDialogRef = this.dialog.open(ConfirmComponent, {
                data: {
                    message: 'You haven\'t saved the changes you made. If you confirm, all your changes will be lost.'
                }
            })
            confirmDialogRef.afterClosed().subscribe((status: boolean) => {
                if (status) {
